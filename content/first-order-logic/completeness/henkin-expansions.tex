...
\begin{defn}[Consistent relative to $\Lang L$]
\ollabel{def:consistent-in-L} A set~$\Gamma$ of !!{sentence}s is \emph{consistent relative to $\Lang L$} iff there is no !!{derivation} $\Gamma \Proves \lfalse$ using only sentences in~$\Lang L$.
\end{defn}
